...
const createUser = async (app, props) => {
  ...
  if (user) throw app.httpErrors.badRequest(`User: ${user.email} already exists! Please Login`)

  const hashedPassword = await app.bcrypt.hash(password)

  return await app.knex('users').insert({ email, role_id, password: hashedPassword })
}
/**
 * * update user email or password
 */
const updateUser = async (app, id, props) => {
  var { email, password, role_id } = props || {}

  if (password) {
    password = await app.bcrypt.hash(password)
  }

  const userID = await app.knex('users').where('id', id).update({ email, password, role_id })
